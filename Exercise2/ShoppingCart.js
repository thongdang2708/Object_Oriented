...
document.querySelector('.back-button-cart').addEventListener('click',function (etv) {
    ...
    document.querySelector('.product-detail-view').classList.add('active');
})
...
function renderCart () {
        ...
        products.forEach((product) => {
            ...
            <div class='item' onclick=insertButton(${product.id})>
...
// Display products in a product detail view

function insertButton (id) {
    document.querySelector('.product-detail-view').classList.add('active');
    products.forEach((product) => {
        if (product.id === id) {
        document.querySelector('.product-detail-view').innerHTML = `
        <div class='product-view'>
        <div class='nav-bar-product-view'>
        <div class='back-to-first' onclick=getBack()>   Takaisin(Back)        </div>
        <div class='shopping-cart-icon-product-view' onclick=goToCart()> <span class='number-items'> </span> <span> Ostoskärrylle (Go To Cart)! </span> <img src="./shopping-cart.png" alt=""> </div>
        </div>
        
        <div class='product-image'>     <img src="${product.image}" alt="" width='100%%' height='100%'>          </div>
        <div class='product-info'>
        <div class='product-name'>    ${product.tyypi}             </div>
        <div class='product-description'>   Kuvaus(Description) : ${product.Available}  </div>
        <div class='product-price'>    Hinta(Price): ${product.price}    </div>
        </div>
...
function getBack () {
    document.querySelector('.product-detail-view').classList.remove('active');
}
